...
object WebcamGpuCrtSim extends App {
  ...
  private var crtKernel: CLKernel = _
  ...
  var evenScanline: Boolean = false
  ...
  val programSource =
    """
      ...
      |}
      |
      |kernel void crt(read_only image2d_t src, write_only image2d_t dest, int evenScanline) {
      |  unsigned int x = get_global_id(0);
      |  unsigned int y = get_global_id(1);
      |
      |  int scanlineHeight = 4;
      |
      |  float4 pixel = read_imagef(src, samplerA, (int2)(x, y - (y % scanlineHeight) + (evenScanline?scanlineHeight/2:0)));
      |
      |  float preBoost = 1.0;
      |  pixel.x = min(1.0, pixel.x * preBoost);
      |  pixel.y = min(1.0, pixel.y * preBoost);
      |  pixel.z = min(1.0, pixel.z * preBoost);
      |
      |  float componentAttenuation = 0.0f;
      |  if (x % 3 != 0) pixel.x = pixel.x * componentAttenuation;
      |  if (x % 3 != 1) pixel.y = pixel.y * componentAttenuation;
      |  if (x % 3 != 2) pixel.z = pixel.z * componentAttenuation;
      |  if (y % 3 == 0) {
      |    if (x / 3 % 2 == y / 3 % 2)
      |      pixel = pixel * 0.75f;
      |  }
      |  float scanlineFactor = min(1.0f, 0.75 + 0.75f * sin(((float)y / (float)scanlineHeight * 6.28 + (evenScanline ? 3.14 : 0))));
      |  pixel = pixel * scanlineFactor;
      |
      |  float postBoost = 2.0;
      |  pixel.x = min(1.0, pixel.x * postBoost);
      |  pixel.y = min(1.0, pixel.y * postBoost);
      |  pixel.z = min(1.0, pixel.z * postBoost);
      |
      |  write_imagef(dest, (int2)(x, y), (float4)pixel);
      |}
      |
  ...
  class Listener extends GLEventListener {
    def init(drawable: GLAutoDrawable): Unit = {
      ...
      if (clContext == null) {
        ...
        crtKernel = program.createCLKernel("crt")
          .putArg(camBuffer)
          .putArg(camBuffer)
    ...
    def display(drawable: GLAutoDrawable): Unit = {
      ...
      evenScanline = !evenScanline
      crtKernel.setArg(2, if (evenScanline) 1 else 0)
      commandQueue
        .putAcquireGLObject(liveBuffer)
        .putAcquireGLObject(backBuffer)
        .putAcquireGLObject(camBuffer)
        .put2DRangeKernel(crtKernel, 0, 0, Width, Height, 0, 0)
